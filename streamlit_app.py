...
# --- 2. AUDIO ENGINE (With Safety Check) ---
def get_audio_html(file_name, play_twice=False):
    try:
        ...
        # Returns a tiny invisible div instead of an empty string to prevent crashing
        return '<div style="display:none;">Audio file missing</div>'
...
with col_bell:
    ...
    if st.button("🔔", help="Play Bell for all", key="global_bell_btn"):
...
if data["history"]:
    ...
    for entry in reversed(data["history"]):
        ...
        table_html += f'<tr style="border-bottom: 1px solid #eee;"><td>{entry["User"]} {icon}</td><td>{entry["Date"]}</td><td>{entry["Start"]}</td><td>{entry["End"]}</td><td>{entry["Duration"]}</td></tr>'
    table_html += '</table><br>'
    st.markdown(table_html, unsafe_allow_html=True)
    
    h1, h2 = st.columns(2)
    with h1:
        if st.button("🗑️ Clear History", key="clr_btn", use_container_width=True):
            data["history"] = []; st.rerun()
    with h2:
        df = pd.DataFrame(data["history"])
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, index=False)
        st.download_button("📥 Xuất Excel", data=output.getvalue(), file_name=f"History_{get_gmt7_time('%Y-%m-%d')}.xlsx", mime="application/vnd.ms-excel", key="dl_btn", use_container_width=True)
else:
    st.info("Chưa có lịch sử.")

# --- 8. AUDIO TRIGGER & RERUN ---
bell_placeholder = st.empty()
alarm_placeholder = st.empty()

# Global Bell Logic
if data["last_global_bell_time"] > st.session_state.local_played_bell_time:
    html = get_audio_html("endBreak.mp3", play_twice=False)
    bell_placeholder.html(html)
...
if st.session_state.alarm_trigger:
    ...
    alarm_placeholder.html(get_audio_html(f, play_twice=True))
